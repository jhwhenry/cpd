...
cpd-cli manage case-download --components=ibm-licensing,scheduler,cpfs,cpd_platform,watsonx_data,watsonx_ai,watsonx_orchestrate,spss,dods,datastage_ent --release=5.3.1
```
...
```bash
export PATCH_ID=4
cpd-cli manage case-download --components=ibm-licensing,scheduler,cpfs,cpd_platform,watsonx_data,watsonx_ai,watsonx_orchestrate,spss,dods,datastage_ent --release=5.3.1 --patch_id=${PATCH_ID}
```

Set `COMPONENTS_TO_PATCH` to the same list of components used above:

```bash
export COMPONENTS_TO_PATCH=ibm-licensing,scheduler,cpfs,cpd_platform,watsonx_data,watsonx_ai,watsonx_orchestrate,spss,dods,datastage_ent
```
